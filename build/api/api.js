...
const loggingFetch = async (input, init) => {
    ...
    const operations = queries
        ? Array.isArray(queries)
            ? queries.map((op) => op.operationName)
...
const createLink = (preview = false, apiToken, excludeInvalid = true) => {
    const headers = {
...
export const apiQuery = async (query, options) => {
...
export const checkIsPaganationQuery = (doc) => {
    //@ts-ignore
    ...
    const { selectionSet, variableDefinitions } = operation;
...
export const apiQueryAll = async (doc, opt = {}, options = { batchSize: 50, delay: 100 }) => {
...
export const SEOQuery = (model, id) => {
